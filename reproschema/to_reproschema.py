...
from .mappings import CSV_TO_REPROSCHEMA_MAP, INPUT_TYPE_MAP, VALUE_TYPE_MAP, ADDITIONAL_NOTES_LIST
...
class ReproSchemaConverter:
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.branch_logic_pattern = re.compile(r"\[([^\]]+)\]|\b(AND|OR)\b|([^><!=])=|sum\(([^)]+)\)")
    ...
    def preprocess_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        special_fields = [col for col in df.columns if col.endswith(('_Validity', '_Administration', '_Informant'))]
        for field in special_fields:
            df[field] = df[field].apply(lambda x: x.replace('&gt;', '>').replace('\n', '').replace('\r', '') if isinstance(x, str) else x)
        return df
    ...
    def process_response_options(self, response_option_str: str, item_name: str) -> tuple:
        ...
        if item_name.endswith(('_Validity', '_Administration', '_Informant')):
            ...
            choices = [{'name': {'en': name}, 'value': value} for value, name in matches]
        else:
            for choice in response_option_str.split('{-}'):
                choice = choice.strip()
                if '=>' in choice:
                    value, name = map(lambda x: x.strip("'").strip('"'), choice.split('=>'))
                    choices.append({'name': {'en': name}, 'value': None if value.lower() == 'null' else value})
                elif choice != "NULL=>''":
    ...
    def process_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        item_data = {
            ...
            "question": {"en": self.clean_html(item[CSV_TO_REPROSCHEMA_MAP["question"]])},
            "ui": {"inputType": INPUT_TYPE_MAP.get(item[CSV_TO_REPROSCHEMA_MAP["inputType"]], "text")},
            "responseOptions": {
                "valueType": [VALUE_TYPE_MAP.get(str(item.get(CSV_TO_REPROSCHEMA_MAP.get("validation", ""), "")).strip(), "xsd:string")],
                "multipleChoice": item[CSV_TO_REPROSCHEMA_MAP["inputType"]] == "Multi-select",
            },
        }
    ...
    def process_dataframe(self, df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        activities = {}
        for activity_name, group in df.groupby(CSV_TO_REPROSCHEMA_MAP["activity_name"]):
            items = [self.process_item(item) for item in group.to_dict("records")]
            activities[activity_name] = {
                ...
                "compute": [
                    ...
                    for item in items
                    if "_score" in item["id"].lower() or item["id"].lower().endswith("_raw")
                ],
            }
        return activities

    def create_activity_schema(self, activity_name: str, activity_data: Dict[str, Any], output_path: Path, redcap_version: str):
        json_ld = {
        ...
        write_obj_jsonld(act, path / f"{activity_name}_schema", contextfile_url=CONTEXTFILE_URL)
        ...
        for item in activity_data["items"]:
            ...
            write_obj_jsonld(Item(**item), item_path, contextfile_url=CONTEXTFILE_URL)
    ...
    def create_protocol_schema(self, protocol_name: str, protocol_data: Dict[str, Any], activities: List[str], output_path: Path):
        protocol_schema = {
            ...
            "ui": {
                ...
                "order": [f"../activities/{activity}/{activity}_schema" for activity in activities],
                "shuffle": False,
            },
        }
        ...
        write_obj_jsonld(Protocol(**protocol_schema), protocol_dir / f"{protocol_name}_schema", contextfile_url=CONTEXTFILE_URL)
    ...
    def clean_output_directories(self, output_path: Path):
        ...
        if output_path.exists():
            ...
            print(f"Output directory does not exist, will be created: {output_path}")
    ...
    def convert(self, csv_file: str, output_path: str):
        try:
            ...
            abs_output_path = Path(output_path) / self.config["protocol_name"].replace(" ", "_")
            ...
            for activity_name, activity_data in activities.items():
                self.create_activity_schema(activity_name, activity_data, abs_output_path, self.config["redcap_version"])

            self.create_protocol_schema(self.config["protocol_name"], self.config, list(activities.keys()), abs_output_path)
